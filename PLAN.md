...
앱 실행 → [카메라] 화면 (기본 시작 화면)
  ...
  │  바깥 영역은 옅은 블러 처리 — 사용자가 폰을 움직여 박스 안에 텍스트를 맞춤
  ├ 하단 [Scan] 버튼을 누르면 인식 시작 → 박스 안 텍스트가 잠시 안정되면 자동 확정
  │  (버튼 없이 계속 인식하던 원안에서 변경 — 의도치 않은 인식 방지. 설계 문서 §8)
  ├ 박스 안 텍스트 인식 (온디바이스, VisionKit DataScannerViewController)
...
| 화면 | 내용 |
|---|---|
| 카메라 (시작 화면) | 실시간 프리뷰, 고정 텍스트 박스 가이드(가로 화면 절반 × 세로 10pt 텍스트 1줄 비율), 박스 밖 옅은 블러 처리, 하단 Scan / Cancel 버튼. 인식 결과는 이 화면 위 바텀시트로 표시(표제어·발음기호·🔊, 품사별 정의, 예문, 문맥 원문) |
