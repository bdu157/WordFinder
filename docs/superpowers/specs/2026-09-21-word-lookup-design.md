...
## 12. 실기기 검증 결과 (2026-09-21, iPhone 15 Pro)

사용자가 직접 확인했고, §10 항목이 모두 정상이었다.

| 항목 | 결과 |
|---|---|
| 단어를 누르면 스피너 → 3초 안에 성공 화면 또는 Apple 사전 | 정상 |
| Apple 사전 위 안내 한 줄 | 정상 |
| **Apple 사전의 "완료" 버튼으로 닫은 뒤** 캡션 복귀, 다시 스캔 시 목록 표시 | **정상** |
| **아래로 쓸어내려 닫은 뒤** 같은 확인 | **정상** |
| 비행기 모드에서 Apple 사전으로 뜻 표시 | 정상 |

가장 큰 위험은 "완료" 버튼이었다. Apple 사전을 우리 시트 안에 끼워 넣었기 때문에, UIKit이 시트를 닫아도 SwiftUI가 알아채지 못하면 스캔 상태가 `settled` 에 갇혀 **다음 스캔 시트가 영영 뜨지 않을** 수 있었다. 두 가지 닫는 방법 모두 확인했고 문제없었다 — §10에서 준비해 둔 대안(Apple 사전을 따로 띄우기)은 필요 없다.

## 13. 다음 단계로 넘기는 것

최종 코드 리뷰에서 나왔고, 2·3단계가 이 코드 위에 올라가므로 여기 남긴다.

**3단계(히스토리 저장)의 선행 조건**

- **취소된 조회를 무시한다.** 지금은 조회가 취소되면(시트가 닫히면) `.unavailable` 로 처리된다. 화면이 이미 사라진 뒤라 지금은 무해하지만, 결과를 저장하기 시작하면 취소된 조회가 "응답 없음"으로 저장된다. `WordLookupModel.load()` 의 `await` 뒤에 `guard !Task.isCancelled else { return }` 한 줄이면 된다.
- **`fetchedAt` 단위 변환을 한 곳에서.** `DictEntry.fetchedAt` 은 초 단위 `Int`, `DictEntryRecord.fetchedAt` 은 `Date` 다.
- **저장 키를 정한다.** `DictEntryRecord.term` 은 유일한 "정규화된 단어"인데, `DictEntry.term` 은 API가 돌려준 `word` 라서 누른 단어와 다를 수 있다.

**2단계(발음 재생)에서 할 것**

- 여러 항목에 걸친 발음기호·음성 우선순위 테스트 — 실제 픽스처는 항목이 하나뿐이라 검증되지 않았다. 기존 합성 JSON(`bank`)에 phonetics를 넣어 추가한다. 2단계가 `audioUrl` 을 처음 읽는 코드다.

**나중에 (별도 작업)**

- 접근성: 닫기 버튼 터치 영역 30×30 (권장 44), `Font.wf*` 가 고정 크기라 Dynamic Type 미대응, "Show all 34" 가 어느 품사인지 VoiceOver에 알리지 않음. 디자인 토큰 수준의 문제라 이번 범위 밖이다.
- 출처 표기에 CC BY-SA 라이선스 링크를 붙일지 — 제품 결정.
